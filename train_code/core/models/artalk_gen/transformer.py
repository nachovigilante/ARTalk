...
class MixedARTalkDecoder(nn.Module):
    def __init__(self, embed_dim, audio_dim, num_heads, depth, patch_nums):
        ...
        self.audio_pos = nn.Parameter(torch.zeros(1, 1, audio_dim))
        nn.init.trunc_normal_(self.audio_pos, mean=0, std=math.sqrt(1 / audio_dim / 3))
    ...
    def forward(self, feat, audio_feat, prev_feat, style_feat):
        ...
        audio_feat = audio_feat + self.audio_pos
...
class SelfCrossAttn(nn.Module):
    def __init__(self, embed_dim, audio_dim, num_heads, drop_path=0.0):
        ...
        self.audio_attn = CrossAttention(embed_dim=embed_dim, context_embed_dim=audio_dim, num_heads=num_heads)
    ...
    def forward(self, feat, audio_feat, attn_rope_pos, audio_rope_pos, self_attn_bias):
        feat = feat + self.drop_path(self.self_attn(feat, attn_rope_pos, self_attn_bias))
        feat = feat + self.drop_path(self.audio_attn(feat, audio_feat, attn_rope_pos, audio_rope_pos))
